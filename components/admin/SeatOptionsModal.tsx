...
export function SeatOptionsModal({
  ...
  const handleViewTicket = (booking: Booking) => {
    if (!booking.boleto?.numeroTicket) {
      toast.error('Este pasajero no tiene boleto disponible. El boleto se genera al confirmar la venta.');
      return;
    }
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-start justify-between">
                          <div className="flex-1 space-y-2">
                            <div className="flex items-center gap-2">
                              <User className="h-4 w-4 text-muted-foreground" aria-hidden="true" />
                              <span className="font-semibold text-lg">{booking.nombrePasajero}</span>
                            </div>
                            ...
                            {booking.boleto?.numeroTicket ? (
                              ...
                              <div className="text-xs text-amber-600 italic">Boleto pendiente de emisión</div>
                            )}
                          </div>
                          <Button
                            variant="outline"
            ...
            {bookings.length === 0 && (
              <Card>
                <CardContent className="p-8 text-center">
                  ...
                  <Button onClick={handleAddNewPassenger} className="mt-4" aria-label="Registrar primera venta">
                    <Plus className="h-4 w-4 mr-2" aria-hidden="true" />
